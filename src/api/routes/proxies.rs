...
pub async fn start_proxy(
    ...
    // This prevents the SOCKS connection from hanging due to sleep latency at the middle hops.
    {
        ...
        println!("[Proxy] Starting Auto-Tune for Session #{}...", id);
        ...
        if !chain_log.is_empty() {
            println!("[Auto-Tune] Woke up pivot chain: {:?}", chain_log);
        }
    }
    ...
    tokio::spawn(async move {
        println!("[Proxy] Started for Session {} (Tunnel: {}, SOCKS: {})", id, tunnel_port, socks_port);
        ...
        println!("[Proxy] Stopped for Session {}", id);
    });
