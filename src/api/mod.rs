...
use crate::database::DbRef;

// --- FIX: Re-export these types publicly so server.rs can access them ---
pub use state::{ApiContext, SharedResults, SharedProxies, SharedScripts};
use crate::api::routes::{hosts, proxies, modules, history};

// Re-export shared types for use in server.rs if you prefer simpler names
// (Optional, but good for backward compatibility if you used ResultsType)
...
pub async fn start_api_server(
    ...
    // Explicitly handle the server future to satisfy type inference
