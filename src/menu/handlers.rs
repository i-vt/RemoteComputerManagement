...
pub fn handle_session(
    ...
    let session = match map.get(&session_id) {
        Some(s) => s,
        None => {
            println!("[-] Session {} lost.", session_id);
            return;
        }
    };
...
fn handle_extension_load(line: &str, session: &Session) {
    ...
    if parts.len() >= 3 {
        ...
        let ext_args: Vec<&str> = parts.iter().skip(3).cloned().collect();
        let path = format!("./extensions/{}.rhai", ext_name);
        
        match fs::read_to_string(&path) {
            Ok(content) => {
                let b64 = BASE64.encode(content);
                let mut cmd = format!("ext:load {}", b64);
                for arg in ext_args {
                    cmd.push(' ');
                    cmd.push_str(arg);
                }
                let _ = session.tx.send((cmd, None));
                println!("[+] Extension '{}' sent.", ext_name);
            },
            Err(_) => println!("[-] Failed to read extension: {}", path),
        }
    } else {
        println!("Usage: extension load <name> [args...]");
    }
}
    ...
    if parts.len() == 3 {
        match file_transfer::read_file_to_b64(parts[1]) {
            Ok((b64, _)) => {
                ...
                let _ = session.tx.send((cmd, None));
                println!("[+] Uploading {} bytes...", b64.len());
            },
            Err(e) => println!("[-] File Error: {}", e),
        }
    } else { println!("Usage: upload <local> <remote>"); }
}
