...
pub fn start(
    ...
    
    {
        ...
        if map.contains_key(&session_id) {
            println!("[-] Proxy already running for session {}.", session_id);
            return;
        }
...
pub fn stop(
    ...
    if let Some(tx) = map.remove(&session_id) {
        ...
        println!("[+] Proxy stopped for session {}.", session_id);
    } else {
        println!("[-] No active proxy found for session {}.", session_id);
    }
}
...
fn run_proxy_runtime(
    ...
    rt.block_on(async move {
        let tunnel_listener = match TcpListener::bind("0.0.0.0:0").await {
            Ok(l) => l,
            Err(e) => { println!("[-] Tunnel Bind Fail: {}", e); return; }
        };
        let socks_listener = match TcpListener::bind("127.0.0.1:0").await {
            Ok(l) => l,
            Err(e) => { println!("[-] SOCKS Bind Fail: {}", e); return; }
        };
        ...
        println!("[i] Tunnel Listening on 0.0.0.0:{}", tunnel_port);
        println!("[i] SOCKS5 Listening on 127.0.0.1:{}", socks_port);

        // Tell client to connect to us
        let _ = session_tx.send((format!("proxy:start {}", tunnel_port), None));
        println!("[Proxy] Waiting for Client connection...");
        ...
        let (stream, addr) = tokio::select! {
            ...
            _ = &mut stop_signal => {
                println!("[Proxy] Stopped before client connection.");
                return;
            }
        };

        println!("[Proxy] Client {} connected via Tunnel.", addr);
        ...
        loop {
            tokio::select! {
                ...
                _ = &mut stop_signal => {
                    println!("[Proxy] Shutdown signal received for Session {}.", session_id);
                    break;
                }
            }
        }
