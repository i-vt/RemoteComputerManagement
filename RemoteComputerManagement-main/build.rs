...
fn main() {
    ...
    // --- 1b. PLACEHOLDER CERTS ---
    // include_bytes!("../certs/{ca.crt,client.crt,client.key.der}") in transport.rs
    // requires these files at compile time. Generate empty placeholders when
    // gen_certs.sh hasn't been run yet so `cargo check` and dev builds succeed.
    // Production builds MUST run ./gen_certs.sh first to get real TLS material.
